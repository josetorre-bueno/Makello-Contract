// contract_tool_app_v0.1.5.jsx
// Wipomo Contract Tool — Center for Community Energy
// v0.1.5 — 2026-04-19
//
// Changes from v0.1.3:
//  - Site photo: file upload widget with thumbnail preview (jpg/png/webp)
//  - Image injected at top of page 1 via PizZip XML (no external image module needed)
//  - a: and pic: namespaces declared inline on injected XML elements
//  - Image auto-scales: max 6" wide, max ¼ page tall (2.25"), aspect-ratio preserved
//  - dataUrlToUint8Array helper; addPhotoToDocx() post-processes the docx blob
//  - Photo placed at top of contract (not as an exhibit)
//  - customer_tax_status_other clears automatically when status changes away from "Other"
//  - Fixed: window.docxtemplater (lowercase) is the correct CDN global name
...
function App() {
  ...
  function exportCsv() {
    ...
    for (const f of FIELDS.filter(fi => fi.type === 'job' && fi.widget !== 'photo')) {
      const cur  = job[f.key] ?? '';
  ...
  async function generateContract() {
    ...
    try {
      ...
      let docxBlob = doc.getZip().generate({
        type: 'blob',
        mimeType: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
      });

      // Inject Exhibit B if a photo was provided
      if (sitePhoto) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f0f4f8' }}>
      ...
      <div style={{ background: '#1a365d', color: 'white', padding: '10px 20px',
        ...
        <span style={{ fontSize: 11, opacity: 0.55 }}>Center for Community Energy · v0.1.5</span>
