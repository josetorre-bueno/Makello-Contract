// contract_tool_app_v0.2.1.jsx
// Wipomo Contract Tool — Center for Community Energy
// v0.2.1 — 2026-04-19
//
// Changes from v0.2.0:
//  - Fix "Styles 1" Word repair warning: strip leftover template comments
//    (comments.xml, commentsExtended.xml, commentsIds.xml) and their XML
//    anchors from the output docx.  When docxtemplater rewrites runs,
//    comment anchor IDs become orphaned and Word "repairs" them on open.
//  - Remove linebreaks:true (none of our field values contain \n; option
//    was doing extra XML processing for no benefit).
//  - Add compression:'DEFLATE' to both ZIP generate calls for clean output.
//  - Photo upload field: add note that photo must be re-uploaded each
//    session (browser security prevents reading files from CSV path).
...
async function addPhotoToDocx(docxBlob, photoData) {
  ...
  // 3. Build image paragraph — centered, no page break, flows into contract below
  const { cx, cy } = calcImageEmu(width, height);
  const topXml = buildImageXml(rId, cx, cy) + '\n<w:p/>';  // one blank line after photo

  // 4. Inject at the very start of <w:body>, before all contract content
...
function PhotoUploadField({ field, photo, onPhotoChange }) {
...
function FieldRow({ field, value, locked, onChange, dimmed, photo, onPhotoChange }) {
  if (field.widget === 'photo')  return <PhotoUploadField field={field} photo={photo} onPhotoChange={onPhotoChange} />;
...
function App() {
  ...
  function exportCsv() {
    ...
    for (const f of FIELDS.filter(fi => fi.type === 'job')) {
      let cur, note;
      if (f.widget === 'photo') {
        // Image data can't go in a CSV — export filename as a reference
  ...
  async function generateContract() {
    setGenerating(true); setStatus('Loading template…'); setDiagErrors([]);
    try {
      ...
      const doc = new window.docxtemplater(zip, {
        delimiters: { start: '{{', end: '}}' },   // template uses double-brace syntax
        // linebreaks and paragraphLoop not needed: no \n in values, no array loops
      });
      const mergeData = normalizeAllValues({ ...allValues, site_photo: '' });
      doc.render(mergeData);

      // Strip leftover template comments — their run anchors become orphaned
      // after docxtemplater rewrites runs, causing Word's "Styles 1" repair warning.
      const outZip = doc.getZip();
      const COMMENT_FILES = [
        'word/comments.xml', 'word/commentsExtended.xml', 'word/commentsIds.xml',
      ];
      COMMENT_FILES.forEach(f => { try { outZip.remove(f); } catch (_) {} });
      // Remove comment anchors from document.xml
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f0f4f8' }}>
      ...
      <div style={{ background: '#1a365d', color: 'white', padding: '10px 20px',
        ...
        <span style={{ fontSize: 11, opacity: 0.55 }}>Center for Community Energy · v0.2.1</span>
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 10, alignItems: 'center' }}>
          {status && <span style={{ fontSize: 12, color: statusIsGood ? '#9ae6b4' : '#feb2b2' }}>{status}</span>}
      ...
      {diagErrors.length > 0 && (
        <div style={{ background: '#fff5f5', borderBottom: '2px solid #fc8181', padding: '10px 20px' }}>
