// contract_tool_app_v0.3.11.jsx
// Makello Contract Tool (Beta)
// v0.3.11 — 2026-05-11
//
// Changes from v0.3.10:
//  - Fix: yellow highlight inserted before </w:rPr> (OOXML schema ordering).
//    Inserting at the start of rPr violated the element sequence and caused
//    Word to report "Styles 1" on open.
//  - Fix: footer injected as footer_pgnum.xml to avoid overwriting any
//    existing footer1.xml already present in the template.
//
// Changes from v0.3.5:
...
// ─────────────────────────────────────────────────────────────────────────────
// Yellow highlight helpers
// ─────────────────────────────────────────────────────────────────────────────

// Add yellow highlight to every <w:r> run that contains {{ or }} so that after
// docxtemplater fills the placeholder, the resulting text is highlighted.
// w:highlight must come near the end of w:rPr per the OOXML schema sequence —
// inserting before </w:rPr> keeps all existing elements in their correct order.
function addYellowHighlightToPlaceholders(xml) {
...
function App() {
  ...
  async function generateContract(isDraft = false) {
    ...
    try {
      ...
      // Optional: add yellow highlight to placeholder runs before rendering so
      // filled values inherit the highlight in the output document.
      if (!cleanCopy) {
        const rawDocXml = renderZip.file('word/document.xml').asText();
        renderZip.file('word/document.xml', addYellowHighlightToPlaceholders(rawDocXml));
      }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f0f4f8' }}>
      ...
      <div style={{ background: '#1a365d', color: 'white', padding: '10px 20px',
        ...
        <span style={{ fontSize: 11, opacity: 0.45 }}>v0.3.11</span>
