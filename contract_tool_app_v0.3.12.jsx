// contract_tool_app_v0.3.12.jsx
// Makello Contract Tool (Beta)
// v0.3.12 — 2026-05-11
//
// Changes from v0.3.11:
//  - Highlighting: switched from pre-render template injection to a
//    post-render sentinel approach. Unicode markers (U+FFF9/FFFA) wrap each
//    filled value before doc.render(); after rendering, only the sentinel-
//    bearing runs get highlighted and the markers are stripped. This ensures
//    only the inserted value is yellow, not surrounding paragraph text.
//  - Singular/plural: paragraphs where a "1" value was filled now get
//    " years" corrected to " year" (covers warranty sections 9.1 and 9.3).
//    Uses the same sentinels so only filled-value "1"s trigger the fix.
//
// Changes from v0.3.5:
...
// Add yellow highlight to runs that contain sentinel markers, then strip them.
// w:highlight goes before </w:rPr> to respect OOXML element ordering.
// Only the value run is highlighted — surrounding paragraph text is untouched.
function applyHighlightAndStripMarkers(xml) {
  xml = xml.replace(/<w:r(?:\s[^>]*)?>[\s\S]*?<\/w:r>/g, run => {
    ...
    return run.replace(/(<w:r(?:\s[^>]*)?>)/, '$1<w:rPr><w:highlight w:val="yellow"/></w:rPr>');
  });
...
// Replaces only the first " years" text run that follows the "1" value within
// the same paragraph, so a "5 years" clause elsewhere is never affected.
function fixSingularYears(xml) {
  ...
  return xml.replace(/<w:p(?:\s[^>]*)?>[\s\S]*?<\/w:p>/g, para => {
    ...
    const after  = para.slice(idx + s1.length)
                       .replace(/<w:t([^>]*)> years<\/w:t>/, '<w:t$1> year</w:t>');
    return before + after;
  });
}
...
function App() {
  ...
  async function generateContract(isDraft = false) {
    ...
    try {
      ...
      saveAs(docxBlob, `${prefix}Wipomo_Contract_${slug}${ds ? '_' + ds : ''}.docx`);
      if (isDraft) {
        setStatus(`✓ Draft prepared — ${missing.length} field${missing.length !== 1 ? 's' : ''} left blank`);
      } else {
        setStatus(
          sitePhoto && includeAddendum ? '✓ Contract + addendum with photo generated' :
          sitePhoto                    ? '✓ Contract with site photo generated' :
          includeAddendum              ? '✓ Contract + addendum generated' :
                                         '✓ Contract generated'
        );
      }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f0f4f8' }}>
      ...
      <div style={{ background: '#1a365d', color: 'white', padding: '10px 20px',
        ...
        <span style={{ fontSize: 11, opacity: 0.45 }}>v0.3.12</span>
        ...
        <div style={{ background: '#ebf8ff', borderBottom: '2px solid #bee3f8', padding: '16px 24px' }}>
          <div style={{ maxWidth: 1100, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 24 }}>
            ...
            <div>
              ...
              <p style={{ margin: 0, fontSize: 12, color: '#2d3748', lineHeight: 1.5 }}>
                <strong>Field highlighting</strong> — by default the generated Word document highlights every filled field in yellow, making it easy for a reviewer to spot what was inserted. Check <strong>Clean Copy</strong> in the header before generating to produce a version without highlighting, suitable for sending to the customer.
              </p>
            </div>
