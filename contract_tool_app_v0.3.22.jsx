...
function isLegacyCsv(rows) {
  ...
  // across row 1.  The first column header is always 'owner_name'.
  // Normal contract_input CSV: col[0] of row 0 is 'description' or 'placeholder'.
...
function parseLegacyCsv(rows) {
  ...
  // Build a lookup of all CSV fields, apply LEGACY_FIELD_MAP for direct mappings,
  // then construct initial_target_capacity from system_size + battery_type.
  ...
  const cap = buildInitialTargetCapacity(raw.system_size, raw.battery_type);
