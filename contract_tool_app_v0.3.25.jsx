...
function buildInitialTargetCapacity(systemSize, batteryType) {
  ...
  if (solar > 0 && battKwh > 0)
    return `${solar} kW DC Solar PV and ${battKwh}kWh ${battKw}kW Storage`;
  if (solar > 0)
    return `${solar} kW DC Solar PV`;
  if (battKwh > 0)
    return `${battKwh}kWh ${battKw}kW Grid Tied Energy Storage`;
  return '';
}

function isLegacyCsv(rows) {
  // Detects Makello wide/horizontal export (type 2 or 3 below) vs the normal
  // contract_input CSV (type 1):
  //   Type 1 — normal contract_input: 2-3 col vertical (description / {{key}} / value)
  //   Type 2 — old Makello export: row 0 = ~100 field names, first col 'owner_name'
  //   Type 3 — new Makello export: same horizontal layout, first col is 'project_type'
  // Fallback: check for multiple known Makello field names anywhere in row 0 in case
  // the column order changes again.
  if (rows.length < 2) return false;
  const first = String(rows[0][0]).trim();
  if (first === 'owner_name' || first === 'project_type') return true;
  const h = new Set(rows[0].map(c => String(c).trim()));
  return ['nrel_pv_watts', 'estimated_total', 'total_flat_fee_adders_with_marketing_fee']
    .filter(f => h.has(f)).length >= 2;
}

function parseLegacyCsv(rows) {
  // Wide/horizontal format: row 0 = field names, row 1 = corresponding values.
  // Build a lookup of all CSV fields, pass through direct key matches, apply
  // LEGACY_FIELD_MAP for renames, then construct initial_target_capacity string.
  ...
  // Pass through any CSV column whose name is already a JOB_KEY (new export format
  // uses contract-tool field names directly, e.g. customer_address, estimated_total).
  for (const key of JOB_KEYS) {
    if (raw[key] !== undefined && raw[key] !== '') out[key] = raw[key];
  }

  // Apply legacy remappings only when the target slot isn't already filled —
  // prevents total_flat_fee_adders_with_marketing_fee from overwriting estimated_total.
  for (const [csvKey, contractKey] of Object.entries(LEGACY_FIELD_MAP)) {
    if (out[contractKey] !== undefined && out[contractKey] !== '') continue;
    const val = raw[csvKey];
    if (val !== undefined && val !== '') out[contractKey] = val;
  }

  // Build human-readable system description.  Old format uses system_size (kW number);
  // new format stores the kW value in initial_target_capacity instead.
  const cap = buildInitialTargetCapacity(raw.system_size || raw.initial_target_capacity || '', raw.battery_type);
  if (cap) out.initial_target_capacity = cap;
...
function App() {
  ...
  async function mergeAddendumInto(outputZip, mergeData) {
    const addResp = await fetch('./addendum_template.docx');
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f0f4f8' }}>
      ...
      <div style={{ background: '#1a365d', color: 'white', padding: '10px 20px',
        ...
        <span style={{ fontSize: 11, opacity: 0.45 }}>v0.3.22</span>
      ...
      {showHelp && (
        <div style={{ background: '#ebf8ff', borderBottom: '2px solid #bee3f8', padding: '16px 24px' }}>
          <div style={{ maxWidth: 1200, display: 'grid', gridTemplateColumns: '1.3fr 1fr 1fr 1fr', gap: 24 }}>
            ...
            <div>
              ...
              <p style={{ margin: '0 0 8px', fontSize: 12, color: '#2d3748', lineHeight: 1.5 }}>
                Click <strong>⬇ Blank CSV</strong> to download a starter file. Fill Column C, save as CSV, then drag-and-drop or click to load. An exported CSV (↑ Export CSV) adds a fourth column recording changes with dates.
              </p>

              <div style={{ fontWeight: 600, fontSize: 12, color: '#2c5282', margin: '0 0 3px' }}>Format 2 — Sky-D export, old schema (auto-detected)</div>
              <p style={{ margin: '0 0 4px', fontSize: 12, color: '#2d3748', lineHeight: 1.5 }}>
                Wide CSV where row 1 starts with <code style={{ background: '#e2e8f0', padding: '1px 3px', borderRadius: 3 }}>owner_name</code>. Fields mapped:
              </p>
              <ul style={{ margin: '0 0 8px', paddingLeft: 16, fontSize: 12, color: '#2d3748', lineHeight: 1.6 }}>
                <li><code style={{ background: '#e2e8f0', padding: '1px 3px', borderRadius: 3 }}>owner_name</code> → Customer name</li>
                <li><code style={{ background: '#e2e8f0', padding: '1px 3px', borderRadius: 3 }}>address</code> → Site address</li>
                <li><code style={{ background: '#e2e8f0', padding: '1px 3px', borderRadius: 3 }}>total_flat_fee_adders_with_marketing_fee</code> → Estimated total</li>
                <li><code style={{ background: '#e2e8f0', padding: '1px 3px', borderRadius: 3 }}>system_size</code> + <code style={{ background: '#e2e8f0', padding: '1px 3px', borderRadius: 3 }}>battery_type</code> → System description</li>
              </ul>

              <div style={{ fontWeight: 600, fontSize: 12, color: '#2c5282', margin: '0 0 3px' }}>Format 3 — Sky-D export, current schema (auto-detected)</div>
              <p style={{ margin: '0 0 4px', fontSize: 12, color: '#2d3748', lineHeight: 1.5 }}>
                Same wide layout but row 1 now starts with <code style={{ background: '#e2e8f0', padding: '1px 3px', borderRadius: 3 }}>project_type</code>. More fields populate directly:
              </p>
              <ul style={{ margin: '0 0 8px', paddingLeft: 16, fontSize: 12, color: '#2d3748', lineHeight: 1.6 }}>
                <li><code style={{ background: '#e2e8f0', padding: '1px 3px', borderRadius: 3 }}>customer_address</code>, <code style={{ background: '#e2e8f0', padding: '1px 3px', borderRadius: 3 }}>customer_tax_status</code></li>
                <li><code style={{ background: '#e2e8f0', padding: '1px 3px', borderRadius: 3 }}>estimated_total</code>, <code style={{ background: '#e2e8f0', padding: '1px 3px', borderRadius: 3 }}>prevailing_wage</code></li>
                <li><code style={{ background: '#e2e8f0', padding: '1px 3px', borderRadius: 3 }}>effective_date</code>, <code style={{ background: '#e2e8f0', padding: '1px 3px', borderRadius: 3 }}>contract_date</code></li>
                <li><code style={{ background: '#e2e8f0', padding: '1px 3px', borderRadius: 3 }}>initial_target_capacity</code> (kW number) → system description built automatically</li>
              </ul>
              ...
              <div style={{ fontSize: 12, color: '#744210', lineHeight: 1.5, background: '#fffbeb',
                ...
                <strong>For both Makello formats:</strong> customer name and signatory fields may still need manual entry if blank in the proposal.
                <br/><br/>
                <strong>Site photo</strong> — a CSV can only store a filename, not the image itself. After loading any Makello CSV, the photo upload widget shows the last-used filename as a reminder, but <strong>the photo must be re-uploaded every session</strong> before generating the contract (browsers cannot read files from your disk without your permission).
              </div>
            </div>
            ...
            <div>
              ...
              <p style={{ margin: 0, fontSize: 12, color: '#2d3748', lineHeight: 1.5 }}>
                The tool automatically loads defaults from the repo copy on startup, then falls back to browser local storage if the file is not reachable.
              </p>
            </div>
            ...
            <div>
              ...
              <p style={{ margin: '0 0 4px', fontSize: 12, color: '#2d3748', lineHeight: 1.5 }}>
                Three fields are calculated automatically:
              </p>
              ...
              <p style={{ margin: '0 0 6px', fontSize: 12, color: '#2d3748', lineHeight: 1.5 }}>
                Dollar amounts are formatted automatically on blur — enter a bare number and <strong>$</strong> is added. Percentage fields add <strong>%</strong>. Values above 1 are treated as already in percent (e.g. <em>8</em> → <em>8%</em>); values ≤ 1 are scaled (e.g. <em>0.08</em> → <em>8%</em>).
              </p>
              <p style={{ margin: 0, fontSize: 12, color: '#2d3748', lineHeight: 1.5 }}>
                <strong>Field highlighting</strong> — by default the generated Word document highlights every filled field in yellow. Check <strong>Clean Copy</strong> in the header to produce a version without highlighting, suitable for sending to the customer.
              </p>
            </div>

            {/* Col 4 — PDF output */}
            <div>
              <div style={{ fontWeight: 700, fontSize: 13, color: '#2c5282', marginBottom: 6 }}>PDF Output</div>
              <p style={{ margin: '0 0 6px', fontSize: 12, color: '#2d3748', lineHeight: 1.5 }}>
                Check <strong>Output PDF</strong> in the header before clicking Generate. Instead of downloading a Word file, the tool will:
              </p>
              <ol style={{ margin: '0 0 8px', paddingLeft: 18, fontSize: 12, color: '#2d3748', lineHeight: 1.8 }}>
                <li>Fill the Word template as normal.</li>
                <li>Convert the filled document to HTML (via mammoth).</li>
                <li>Open a new browser tab with the contract rendered in print-ready CSS.</li>
                <li>Automatically open the <strong>browser print dialog</strong>.</li>
                <li>Choose <strong>Save as PDF</strong> (or the equivalent on your OS) and click Save.</li>
              </ol>
              <p style={{ margin: '0 0 6px', fontSize: 12, color: '#2d3748', lineHeight: 1.5 }}>
                This produces a <strong>real text PDF</strong> — fully searchable, selectable, and copyable — not a scanned image. Formatting (fonts, heading colours, image size) matches the Word original closely.
              </p>
              <p style={{ margin: '0 0 6px', fontSize: 12, color: '#2d3748', lineHeight: 1.5 }}>
                <strong>Recommended PDF viewer:</strong> Mac <strong>Preview</strong> opens browser-generated PDFs without issue. Adobe Reader may show a "damaged file" warning on some systems — this is a compatibility quirk, not actual data loss; the file is valid. If Adobe Reader rejects it, open it in Preview instead.
              </p>
              <p style={{ margin: 0, fontSize: 12, color: '#744210', lineHeight: 1.5 }}>
                The PDF tab must remain open until the print dialog closes. Do not close it early or the PDF will not save.
              </p>
            </div>
