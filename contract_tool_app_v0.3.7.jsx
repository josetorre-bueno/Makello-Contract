// contract_tool_app_v0.3.7.jsx
// Makello Contract Tool
// v0.3.7 — 2026-05-03
//
// Changes from v0.3.6:
//  - "Prepare Draft" button appears in header when any required fields are
//    missing. Generates the contract immediately, substituting blank underline
//    lines (___________) for every empty required field. Filename is prefixed
//    with DRAFT_. The regular Generate Contract button still blocks and lists
//    the missing fields as before.
//
// Changes from v0.3.5:
...
function App() {
  ...
  async function generateContract(isDraft = false) {
    ...
    try {
      ...
      // 5a. Optionally merge addendum
      if (includeAddendum) {
        setStatus('Merging addendum…');
        await mergeAddendumInto(outputZip, mergeData);
      }

      // 5b. Generate — no compression override so PizZip preserves each file's
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f0f4f8' }}>
      ...
      <div style={{ background: '#1a365d', color: 'white', padding: '10px 20px',
        ...
        <span style={{ fontSize: 11, opacity: 0.45 }}>v0.3.7</span>
