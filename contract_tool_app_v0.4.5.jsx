// contract_tool_app_v0.4.5.jsx
// Makello Contract Tool (Beta)
// v0.4.5 — 2026-05-27
//
// Changes from v0.4.4:
//  - Cache-busting: all template fetch URLs now include ?v=0.4.5 so browsers
...
const LS_TRANSLATION_KEY = 'wipomo_translation_v2';
...
async function exportTranslationCsv(translation) {
  const [mapPv, mapBat] = await Promise.all([
    scanTemplateForContext('./Wipomo_Contract_Template.docx?v=0.4.5',         'PV Only'),
    scanTemplateForContext('./Wipomo_Contract_Template_Battery.docx?v=0.4.5', 'PV+Battery'),
  ]);
  ...
  const commentRow = [`# Wipomo Contract Tool v0.4.5 — translation table exported ${exportDate}`];
...
function App() {
  ...
  async function mergeAddendumInto(outputZip, mergeData) {
    const addResp = await fetch('./addendum_template.docx?v=0.4.5');
  ...
  async function generateContract(isDraft = false) {
    ...
    try {
      const templateFile = contractType === 'pv_battery'
        ? './Wipomo_Contract_Template_Battery.docx?v=0.4.5'
        : './Wipomo_Contract_Template.docx?v=0.4.5';
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f0f4f8' }}>
      ...
      <div style={{ background: '#1a365d', color: 'white', padding: '10px 20px',
        ...
        <span style={{ fontSize: 11, opacity: 0.45 }}>v0.4.5</span>
