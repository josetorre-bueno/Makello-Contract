// contract_tool_app_v0.4.9.jsx
// Makello Contract Tool (Beta)
// v0.4.9 — 2026-05-28 13:21 PT
...
async function exportTranslationCsv(translation) {
  const [mapPv, mapBat] = await Promise.all([
    scanTemplateForContext('./Wipomo_Contract_Template.docx?v=0.4.9',         'PV Only'),
    scanTemplateForContext('./Wipomo_Contract_Template_Battery.docx?v=0.4.9', 'PV+Battery'),
  ]);
  ...
  const commentRow = [`# Wipomo Contract Tool v0.4.9 — translation table exported ${exportDate}`];
...
function App() {
  ...
  useEffect(() => {
    ...
    fetch('./contract_defaults.json?v=0.4.9')
  ...
  async function mergeAddendumInto(outputZip, mergeData) {
    const addResp = await fetch('./addendum_template.docx?v=0.4.9');
  ...
  async function generateContract(isDraft = false) {
    ...
    try {
      const templateFile = contractType === 'pv_battery'
        ? './Wipomo_Contract_Template_Battery.docx?v=0.4.9'
        : './Wipomo_Contract_Template.docx?v=0.4.9';
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f0f4f8' }}>
      ...
      <div style={{ background: '#1a365d', color: 'white', padding: '10px 20px',
        ...
        <span style={{ fontSize: 11, opacity: 0.45 }}>v0.4.9</span>
