// contract_tool_app_v0.5.0.jsx
// Makello Contract Tool (Beta)
// v0.5.0 — 2026-05-28 15:49 PT
// Part of: Makello Contract Tool
//
// MINOR BUMP — architecture change: translation table is now shipped via
// translation_defaults.csv (fetched from the repo on mount) rather than via
// localStorage + DEFAULT_TRANSLATION. Per-user upload of translation tables
// has been removed deliberately — translations are admin-edited, deployed,
// and apply org-wide.
//
// Changes from v0.4.9:
...
function evalExpression(expr, csvRow, evalErrors) {
  expr = String(expr || '').trim();
  if (!expr) return '';

  // Short-circuit: a bare field reference like '[some_field]' is a value
  // fetch, not an arithmetic context. Return the raw CSV value as-is.
  const bare = expr.match(/^\[([^\]]+)\]$/);
  if (bare) {
    const v = csvRow[bare[1]];
  ...
  function getNum(name) {
    ...
    // A real number: optional sign, digits, optional decimal portion.
    // parseFloat is too lenient — it returns 6 for "6/4/26" — so we
    // require the WHOLE stripped string to match the numeric pattern.
    if (/^-?\d+(\.\d+)?$/.test(stripped)) return parseFloat(stripped);
    // Field is referenced inside an arithmetic expression but its value
    // is not a number. Record the failure so the user can see both the
    // expression (translation table problem) and the raw value (input
    // data problem) and judge which side to fix.
    if (evalErrors) {
      evalErrors.push({
        expression: expr,
        field: name,
        rawValue: String(raw),
        message: `Field [${name}] = "${raw}" is not numeric — used in expression "${expr}"`,
      });
    }
    return 0;
  }
...
function App() {
  ...
  useEffect(() => {
    ...
    fetch('./contract_defaults.json?v=0.5.0')
  ...
  // pretending to work.
  useEffect(() => {
    fetch('./translation_defaults.csv?v=0.5.0')
  ...
  async function mergeAddendumInto(outputZip, mergeData) {
    const addResp = await fetch('./addendum_template.docx?v=0.5.0');
  ...
  async function generateContract(isDraft = false) {
    ...
    try {
      const templateFile = contractType === 'pv_battery'
        ? './Wipomo_Contract_Template_Battery.docx?v=0.5.0'
        : './Wipomo_Contract_Template.docx?v=0.5.0';
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f0f4f8' }}>
      ...
      <div style={{ background: '#1a365d', color: 'white', padding: '10px 20px',
        ...
        <span style={{ fontSize: 11, opacity: 0.45 }}>v0.5.0</span>
