...
//    both templates render the guarantee via docxtemplater conditional blocks
//    {{#include_guarantee}}…{{/include_guarantee}} — Article 1 defs, §2.3.1
//    Productive Capacity Controls, §3.2 hook, §9.4/§9.5, and Exhibit C (a
//    30-row Annual Production Potential Table built by buildProductionTable()
//    and emitted as a {{#production_table}} row-loop). No new templates: 2
//    templates (PV / PV+battery) × the boolean cover all 4 combinations.
//  - generateContract now sets docxtemplater { paragraphLoop: true } and passes
//    include_guarantee (bool) + production_table (array) into renderData.
//  - New guaranteeOnly FIELDS: target_annual_production_kwh (per-job, fixed at
//    signing) + company-policy defaults production_floor_pct 90%,
//    first_year_degradation_pct 1.5%, annual_degradation_pct 0.33333%,
//    array_increase_cap_pct 25%, audit_interval_years 3,
//    remedy_completion_months 8, cleaning_visit_fee $350,
//    cleaning_per_module_fee $10. getMissingFields skips guaranteeOnly when off.
//  - UX: default contract type is now PV + Battery and guarantee defaults ON
//    (users skipping the toggles was a recurring problem); the page background
//    takes a soft tint keyed to (contract type × guarantee) as a constant,
//    learnable cue, plus a small mode label in the header.
//  - Cache-bust strings bumped 0.6.8 → 0.7.0.
//
// Changes from v0.6.7:
//  - Provenance footer: every generated contract now carries a small grey
//    "Makello vX.Y.Z · YYYY-MM-DD" stamp on the left of the footer (page
//    number stays on the right), satisfying the global rule that output files
//    record the tool version. Date is the generation date. Sourced from a new
//    single VERSION constant (also drives the on-screen badge).
//  - effective_date is now an at-signing field (was 'optional') in BOTH the
//    JSX FIELDS array (fillStatus) and translation_defaults.csv
//    (required_status), so it is intentionally left blank (___) to be filled
//    by hand at execution, and shows the deep-blue "at signing" status.
//  - Cache-bust strings bumped 0.6.7 → 0.6.8.
//
// Changes from v0.6.6:
//  - BUG FIX (addendum numbering): mergeAddendumInto appended the addendum's
//    <w:abstractNum> elements AFTER the main document's <w:num> elements in
//    numbering.xml. That is well-formed XML but violates the OOXML CT_Numbering
//    schema, which requires ALL <w:abstractNum> before ALL <w:num>. Word
//    enforces the order: it discarded the numbering definitions and fell back
//    to default continuous decimal numbering, so every bulleted list in an
//    addendum-merged contract lost its bullets and renumbered continuously.
//    (The PDF/mammoth path ignores numbering.xml, so it looked fine there.)
//    Fix: insert addendum abstractNums before the first main <w:num>, and
//    append addendum nums before </w:numbering> — preserving schema order.
//  - Cache-bust strings bumped 0.6.6 → 0.6.7.
//
// Changes from v0.6.5:
...
const FIELDS = [
  { key: 'effective_date',                    label: 'Contract execution date (e.g. April 17 2026)',                          type: 'job',    fillStatus: 'at_signing' },
  ...
  // ── Production Guarantee fields (v0.7.0) — only used/required when the
  //    "Include Production Guarantee" toggle is on (guaranteeOnly: true). ──
  { key: 'target_annual_production_kwh',      label: 'Target Annual Production — contracted Year-1 kWh (fixed at signing)',    type: 'job',    fillStatus: 'manual',     guaranteeOnly: true },
  { key: 'production_floor_pct',              label: 'Production guarantee floor (% of Expected)',                            type: 'stable', dflt: '90%', unit: 'pct', guaranteeOnly: true },
  { key: 'first_year_degradation_pct',        label: 'First-year production reduction',                                       type: 'stable', dflt: '1.5%',     guaranteeOnly: true },
  { key: 'annual_degradation_pct',            label: 'Annual production reduction (year 3+)',                                 type: 'stable', dflt: '0.33333%', guaranteeOnly: true },
  { key: 'array_increase_cap_pct',            label: 'Remedy cap — max added capacity (% of nameplate)',                      type: 'stable', dflt: '25%', unit: 'pct', guaranteeOnly: true },
  { key: 'audit_interval_years',              label: 'Audit interval (years) — wait, spacing, and evaluation window',         type: 'stable', dflt: '3',        guaranteeOnly: true },
  { key: 'remedy_completion_months',          label: 'Months to complete remedy construction',                               type: 'stable', dflt: '8',        guaranteeOnly: true },
  { key: 'cleaning_visit_fee',                label: 'Cleaning charge per visit',                                             type: 'stable', dflt: '$350', unit: 'usd', guaranteeOnly: true },
  { key: 'cleaning_per_module_fee',           label: 'Cleaning charge per module',                                            type: 'stable', dflt: '$10',  unit: 'usd', guaranteeOnly: true },
];
...
function addPageNumbersToZip(zip, stampText = '') {
  const RID   = 'rIdPgFt1';
  const FNAME = 'footer_pgnum.xml';
  if (zip.file('word/document.xml').asText().includes(RID)) return; // already present

  // Provenance stamp (small grey) left-aligned; page number right via a right
  // tab at 9360 twips (6.5" — letter with 1" margins). One footer applies to
  // every page, so the stamp appears on all pages.
  const stamp = String(stampText).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  const footerXml = [
    '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>',
    '<w:ftr xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">',
    '<w:p><w:pPr><w:tabs><w:tab w:val="right" w:pos="9360"/></w:tabs></w:pPr>',
    stamp ? `<w:r><w:rPr><w:color w:val="808080"/><w:sz w:val="16"/></w:rPr><w:t xml:space="preserve">${stamp}</w:t></w:r>` : '',
    '<w:r><w:tab/></w:r>',
...
function getMissingFields(allValues, translation, contractType, includeGuarantee) {
  const ct = contractType || 'pv_only';
  return FIELDS.filter(f => {
    if (f.type !== 'job') return false;
    if (f.guaranteeOnly && !includeGuarantee) return false;   // guarantee off → not required
...
// Build the 30-row Annual Production Potential Table (Exhibit C) from the
// contracted Year-1 target and the degradation curve. Year 1 = target;
// Year 2 = target × (1 − first-year reduction); Year n≥3 = prior × (1 − annual).
function buildProductionTable(targetRaw, firstYrRaw, annualRaw) {
  const num = s => { const n = parseFloat(String(s == null ? '' : s).replace(/[^0-9.]/g, '')); return isFinite(n) ? n : NaN; };
  const pct = s => { const n = num(s); return isFinite(n) ? n / 100 : 0; };   // '1.5%' → 0.015
  const target = num(targetRaw), firstYr = pct(firstYrRaw), annual = pct(annualRaw);
  const rows = [];
  let e = 1;
  for (let y = 1; y <= 30; y++) {
    if (y === 2) e = 1 - firstYr;
    else if (y >= 3) e = e * (1 - annual);
    const kwh = (isFinite(target) && target > 0) ? Math.round(target * e) : null;
    rows.push({ year: String(y),
                kwh: kwh == null ? '—' : kwh.toLocaleString('en-US'),
                pct: (e * 100).toFixed(3) + '%' });
  }
  return rows;
}
...
function App() {
  ...
  const [contractType,   setContractType]   = useState('pv_battery'); // 'pv_only' | 'pv_battery' — default PV+Battery (v0.7.0)
  ...
  const [includeGuarantee, setIncludeGuarantee] = useState(true); // default ON (v0.7.0)
  ...
  async function mergeAddendumInto(outputZip, mergeData) {
    ...
    if (mainNumXml && addNumXml) {
      ...
      // Extract abstractNum and num elements from addendum and merge into main
      // numbering.xml. OOXML CT_Numbering requires the sequence
      //   <w:numPicBullet>* <w:abstractNum>* <w:num>*
      // i.e. ALL <w:abstractNum> must precede ALL <w:num>. Appending the
      // addendum's abstractNums after the main <w:num> elements is well-formed
      // XML but schema-invalid; Word discards the numbering and falls back to
      // default continuous decimal numbering (bullets vanish). So we insert the
      // addendum abstractNums BEFORE the first main <w:num>, and the addendum
      // nums at the end — keeping abstractNums and nums each grouped in order.
      ...
      const absBlock = addAbstractNums.length ? addAbstractNums.join('\n') + '\n' : '';
      const numBlock = addNums.length ? addNums.join('\n') + '\n' : '';
      const firstNumMatch = mainNumXml.match(/<w:num\b/);
      if (firstNumMatch) {
        const idx = firstNumMatch.index;
        mainNumXml = mainNumXml.slice(0, idx) + absBlock + mainNumXml.slice(idx);
      } else {
        // No existing <w:num> — place abstractNums before the closing tag.
        mainNumXml = mainNumXml.replace('</w:numbering>', absBlock + '</w:numbering>');
      }
      mainNumXml = mainNumXml.replace('</w:numbering>', numBlock + '</w:numbering>');
  ...
  async function generateContract(isDraft = false) {
    ...
    const missing = getMissingFields(allValues, translation, contractType, includeGuarantee);
    ...
    try {
      ...
      const doc = new window.docxtemplater(renderZip, {
        delimiters: { start: '{{', end: '}}' },
        paragraphLoop: true,   // v0.7.0: needed for {{#include_guarantee}} blocks spanning paragraphs
      });
      ...
      // v0.7.0: optional Production Guarantee — boolean condition + Exhibit C row-loop.
      // Both bypass the sentinel-wrapping above (boolean / array), so the generated
      // table is not highlighted; the guarantee's stable text values still are.
      renderData.include_guarantee = includeGuarantee;
      renderData.production_table = includeGuarantee
        ? buildProductionTable(mergeData.target_annual_production_kwh,
      ...
      // 5a. Add page numbers + provenance stamp to footer (generation date)
      const genDate = new Date().toISOString().slice(0, 10);   // YYYY-MM-DD
      addPageNumbersToZip(outputZip, `Makello ${VERSION} · ${genDate}`);
  ...
  return (
    <div style={{ minHeight: '100vh', background: bgTint, transition: 'background .35s ease' }}>
      ...
      <div style={{ background: '#1a365d', color: 'white', padding: '10px 20px',
        ...
        <span style={{ fontSize: 11, opacity: 0.45 }}>{VERSION}</span>
